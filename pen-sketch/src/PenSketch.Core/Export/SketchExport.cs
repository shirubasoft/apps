...
public static class SketchExport
{
    ...
    {
        using var surface = SKSurface.Create(new SKImageInfo(document.Width * scale, document.Height * scale));
    ...
    public static void Gif(SketchDocument document, Stream output, CancellationToken cancellationToken = default)
    {
        ...
        using var animation = Image.Load<Rgba32>(Png(document, showTrigger: true, scale: 1));
        ...
        for (var i = 0; i <= frames; i++)
        {
            ...
            using var frame = Image.Load<Rgba32>(Png(document, i / (float)frames, true, 1));
