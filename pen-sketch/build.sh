...
rm -f "$artifact_root/PenSketch.apk" "$artifact_root/release-version.txt"
mkdir -p "$test_results" "$coverage_root" "$android_output"

cd "$repo_root"
./test.sh

dotnet build "$repo_root/src/PenSketch.AppHost/PenSketch.AppHost.csproj" --configuration "$configuration"

dotnet publish "$repo_root/src/PenSketch/PenSketch.csproj" \
  --configuration "$configuration" --framework net11.0-android \
...
cp "$apk_path" "$artifact_root/PenSketch.apk"
if [[ -n "$release_version" ]]; then
  printf '%s\n' "$release_version" > "$artifact_root/release-version.txt"
fi
echo "APK: $artifact_root/PenSketch.apk"
