...
`./build.sh` builds the development AppHost, runs 104 data-driven tests and packages the signed Release APK. The APK was installed in a Wear OS 5 emulator with a round 450 × 450 display at 320 dpi. The screenshots and recording use controlled match data from installed Release builds.
...
| Early release cancels; confirmation sinks the deleted score into the trash; navigation afterward | [Deletion recording](https://github.com/user-attachments/assets/3f532ac4-aa7a-4e5c-9e88-fa0092b3798e) |
...
The recording shows the deleted score moving downward and fading behind the raised semicircle before the remaining match appears. Backgrounding during either the exit or reveal restored the correct match at full opacity and its resting position, with point and undo controls working. Completed and interrupted transitions matched a reference capture of the remaining match pixel for pixel. The same comparison passed with animations disabled and with font scale 1.3 in dark theme. The emulator's crash log was empty after the checks.
